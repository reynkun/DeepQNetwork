...
X_state = tf.placeholder(tf.float32, shape=[None, 
                                            input_height, 
                                            input_width,
                                            num_frames_per_input])
...
def run_game(env, q_values_op, n_max_steps=10000):
    input_frames = deque(maxlen=num_frames_per_input)
    frames = []
    total_rewards = 0
    obs = env.reset()
    frames.append(obs)
        ...
        if info is not None and info['ale.lives'] != last_lives:
            ...
            if step % skip_frames == 0:
                ...
                if len(input_frames) >= 4:
                    state = np.concatenate(input_frames, axis=2)
...
# min_memory_before_training = 10000  # start training after 10,000 game iterations
min_memory_before_training = 1000  # start training after 10,000 game iterations
...
checkpoint_path = "./{}-dqn".format(game_id)
    ...
    try:
        while True:
            ...
            if iteration % 1000 == 0:
                ...
                epsilon = max(eps_min, eps_max - (eps_max-eps_min) * step/eps_decay_steps)
                print("{} Iteration {} Training step {}/{}  ({:.1f})% ".format(ts, 
                                                                               iteration, 
                                                                               step, 
                                                                               n_steps,
                                                                               step * 100 / n_steps) + 
                      "Loss {:.3f} Mean Max-Q {:.3f} Mem Len: {} ".format(loss_val, 
                                                                          mean_max_q, 
                                                                          len(replay_memory)) + 
                      "Games played: {:d} scores: {:0.2f} eps: {:0.3f}".format(num_games_played, 
                                                                      sum(game_scores)/len(game_scores), epsilon))
            ...
            if done:
                ...
                game_scores.append(cur_game_score)
                cur_game_score = 0
                num_games_played += 1
            ...
            obs, reward, done, info = env.step(action)
            total_rewards += reward
            cur_game_score += reward

            if not done and (iteration-1) % skip_frames != 0:
                continue

            frame = preprocess_observation(obs)
            input_frames.append(frame)

            if not done and len(input_frames) < num_frames_per_input:
                continue
            ...
            if info is not None and info['ale.lives'] != last_lives:
                # If start of new ball set action to 1
                last_lives = info['ale.lives']
                action = 1
            else:
                # Online DQN evaluates what to do
            ...
            if last_state is not None and last_action is not None:
                replay_memory.append((last_state, 
                                      last_action, 
                                      total_rewards, 
                                      state, 
                                      1.0 - done))
            ...
            if len(replay_memory) < min_memory_before_training:
                continue 
